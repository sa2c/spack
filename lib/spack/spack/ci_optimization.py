...
def matches(obj, proto):
    ...
    if isinstance(obj, collections_abc.Sequence) and not isinstance(obj, str):
        ...
        if not (
            isinstance(proto, collections_abc.Sequence) and not isinstance(proto, str)
        ):
            return False
...
def subkeys(obj, proto):
    ...
    """
    if not (
        isinstance(obj, collections_abc.Mapping)
        and isinstance(proto, collections_abc.Mapping)
    ):
        return obj
