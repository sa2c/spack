    ...
    for arch in ("linux", "darwin"):
        for shared_lib in (None, "/spack/libbar.so"):
            ...
            assert set(output.split()) == set(
                expected[arch]
                .format(static_lib, shared_lib, os.path.basename(shared_lib))
                .split()
            )
...
def test_wrapper_variables(
    ...
    try:
        ...
        assert normpaths(rpath_dir_var.split(":")) == normpaths(
            root_libdirs + dep_lib_dirs
        )
...
def test_external_prefixes_last(
    mutable_config, mock_packages, working_env, monkeypatch
):
...
def test_parallel_false_is_not_propagating(config, mock_packages):
    ...
    for spec in s.traverse():
        expected_jobs = (
            spack.config.get("config:build_jobs") if s.package.parallel else 1
        )
        m = AttributeHolder()
...
def test_build_jobs_sequential_is_sequential():
    assert (
        determine_number_of_jobs(
            parallel=False, command_line=8, config_default=8, max_cpus=8
        )
        == 1
    )
...
def test_build_jobs_command_line_overrides():
    assert (
        determine_number_of_jobs(
            parallel=True, command_line=10, config_default=1, max_cpus=1
        )
        == 10
    )
    assert (
        determine_number_of_jobs(
            parallel=True, command_line=10, config_default=100, max_cpus=100
        )
        == 10
    )
...
def test_build_jobs_defaults():
    assert (
        determine_number_of_jobs(
            parallel=True, command_line=None, config_default=1, max_cpus=10
        )
        == 1
    )
    assert (
        determine_number_of_jobs(
            parallel=True, command_line=None, config_default=100, max_cpus=10
        )
        == 10
    )
