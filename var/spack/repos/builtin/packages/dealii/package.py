...
class Dealii(CMakePackage, CudaPackage):
    ...
    depends_on(
        "trilinos+amesos+aztec+epetra+ifpack+ml+muelu+sacado+teuchos~hypre",
        when="+trilinos+mpi+int64~cuda",
    )
    # TODO: temporary disable Tpetra when using CUDA due to
    # namespace "Kokkos::Impl" has no member "cuda_abort"
    depends_on(
        "trilinos@master+amesos+aztec+epetra+ifpack+ml+muelu+rol+sacado+teuchos~amesos2~ifpack2~intrepid2~kokkos~tpetra~zoltan2",  # noqa
        when="+trilinos+mpi~int64+cuda",
    )
    depends_on(
        "trilinos@master+amesos+aztec+epetra+ifpack+ml+muelu+rol+sacado+teuchos~hypre~amesos2~ifpack2~intrepid2~kokkos~tpetra~zoltan2",  # noqa
        when="+trilinos+mpi+int64+cuda",
    )
    ...
    patch(
        "https://github.com/dealii/dealii/commit/a89d90f9993ee9ad39e492af466b3595c06c3e25.patch",
    ...
    patch(
        "https://github.com/dealii/dealii/commit/f8de8c5c28c715717bf8a086e94f071e0fe9deab.patch",
    ...
    patch(
        "https://github.com/dealii/dealii/commit/80b13fe5a2eaefc77fa8c9266566fa8a2de91edf.patch",
    ...
    patch(
        "https://github.com/dealii/dealii/commit/3b815e21c4bfd82c792ba80e4d90314c8bb9edc9.patch",
    ...
    conflicts(
        "+adol-c",
        when="^trilinos+netcdf",
        msg="Symbol clash between the ADOL-C library and " "Trilinos Netcdf.",
    )
    conflicts(
        "+adol-c",
        when="^trilinos+pnetcdf",
        msg="Symbol clash between the ADOL-C library and " "Trilinos parallel Netcdf.",
    )
