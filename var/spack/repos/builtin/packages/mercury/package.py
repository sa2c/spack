...
class Mercury(CMakePackage):
    ...
    variant(
        "udreg", default=False, description="Enable udreg on supported Cray platforms"
    )
    ...
    def cmake_args(self):
        ...
        if "@1.0.0:" in spec:
            cmake_args.extend(
                [
                    "-DMERCURY_USE_SYSTEM_BOOST:BOOL=%s" % variant_bool("+boostsys"),
                    "-DNA_USE_OFI:BOOL=%s" % variant_bool("+ofi"),
                ]
            )

        if "+ofi" in spec:
            cmake_args.append("-DNA_OFI_GNI_USE_UDREG:BOOL=%s" % variant_bool("+udreg"))
            if self.run_tests:
                cmake_args.append(
