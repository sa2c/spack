...
class CachedCMakePackage(CMakePackage):
    ...
    def initconfig_compiler_entries(self):
        ...
        if "xlf" in (self.compiler.fc or ""):  # noqa: F821
            entries.append(
                cmake_cache_string("CMAKE_Fortran_COMPILER_ID", "XL", familymsg)
            )
        if "xlc" in self.compiler.cc:  # noqa: F821
            entries.append(cmake_cache_string("CMAKE_C_COMPILER_ID", "XL", familymsg))
        if "xlC" in self.compiler.cxx:  # noqa: F821
            entries.append(cmake_cache_string("CMAKE_CXX_COMPILER_ID", "XL", familymsg))
