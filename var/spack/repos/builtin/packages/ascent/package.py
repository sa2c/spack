...
class Ascent(Package, CudaPackage):
    ...
    def install(self, spec, prefix):
        ...
        with working_dir("spack-build", create=True):
            ...
            host_cfg_fname = self.create_host_config(spec, prefix, py_site_pkgs_dir)
            cmake_args = []
    ...
    def create_host_config(self, spec, prefix, py_site_pkgs_dir=None):
        ...
        host_cfg_fname = "%s-%s-%s-ascent.cmake" % (
            socket.gethostname(),
            sys_type,
            spec.compiler,
        )
        ...
        if "+python" in spec and "+shared" in spec:
            ...
            if py_site_pkgs_dir:
                cfg.write(
                    cmake_cache_entry("PYTHON_MODULE_INSTALL_PREFIX", py_site_pkgs_dir)
                )
        else:
