...
class PyAzuremlSdk(Package):
    ...
    depends_on(
        "py-azureml-dataset-runtime@1.23.0:1.23.999 +fuse",
        when="@1.23.0",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-train@1.23.0:1.23.999", when="@1.23.0", type=("build", "run")
    )
    depends_on(
        "py-azureml-train-automl-client@1.23.0:1.23.999",
        when="@1.23.0",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-pipeline@1.23.0:1.23.999", when="@1.23.0", type=("build", "run")
    )

    depends_on("py-azureml-core@1.11.0:1.11.999", when="@1.11.0", type=("build", "run"))
    depends_on(
        "py-azureml-dataset-runtime@1.11.0:1.11.999 +fuse",
        when="@1.11.0",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-train@1.11.0:1.11.999", when="@1.11.0", type=("build", "run")
    )
    depends_on(
        "py-azureml-train-automl-client@1.11.0:1.11.999",
        when="@1.11.0",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-pipeline@1.11.0:1.11.999", when="@1.11.0", type=("build", "run")
    )
    ...
    depends_on(
        "py-azureml-train-automl-client@1.8.0:1.8.999",
        when="@1.8.0",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-pipeline@1.8.0:1.8.999", when="@1.8.0", type=("build", "run")
    )
    depends_on(
        "py-azureml-dataprep@1.8.0:1.8.999 +fuse", when="@1.8.0", type=("build", "run")
    )
