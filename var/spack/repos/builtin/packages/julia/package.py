...
class Julia(Package):
    ...
    version(
        "1.5.1",
        sha256="1d0debfccfc7cd07047aa862dd2b1a96f7438932da1f5feff6c1033a63f9b1d4",
    )
    version(
        "1.5.0",
        sha256="4a6ffadc8dd04ca0b7fdef6ae203d0af38185e57b78f7c0b972c4707354a6d1b",
    )
    version(
        "1.4.2",
        sha256="948c70801d5cce81eeb7f764b51b4bfbb2dc0b1b9effc2cb9fc8f8cf6c90a334",
    )
    version(
        "1.4.1",
    ...
    version(
        "1.4.0",
        sha256="880c73a08296ce8d94ad9605149f2a2b2b028e7202a700ef725da899300b8be9",
    )
    version(
        "1.3.1",
    ...
    version(
        "1.0.0",
        sha256="1a2497977b1d43bb821a5b7475b4054b29938baae8170881c6b8dd4099d133f1",
    )
    version(
        "0.6.2",
    ...
    version(
        "0.5.2",
        sha256="f5ef56d79ed55eacba9fe968bb175317be3f61668ef93e747d76607678cc01dd",
    )
    version(
        "0.5.1",
    ...
    version(
        "0.4.7",
        sha256="d658d5bd5fb79b19f3c01cadb9aba8622ca8a12a4b687acc7d99c21413623570",
    )
    version(
        "0.4.6",
    ...
    version(
        "0.4.5",
        sha256="cbf361c23a77e7647040e8070371691083e92aa93c8a318afcc495ad1c3a71d9",
    )
    version(
        "0.4.3",
        sha256="2b9df25a8f58df8e43038ec30bae195dfb160abdf925f3fa193b59d40e4113c5",
    )
    ...
    conflicts(
        "%intel",
        when="~mkl",
        msg="Building with the Intel compiler requires the mkl variant " "(+mkl)",
    )
    ...
    def install(self, spec, prefix):
        ...
        if "@:0.5.0" in spec:
            options += ["override USE_SYSTEM_CURL=1"]

        if "+cxx" in spec:
            options += ["BUILD_LLVM_CLANG=1", "LLVM_ASSERTIONS=1", "USE_LLVM_SHLIB=1"]
        if spec.target.family == "aarch64":
            options += ["JULIA_CPU_TARGET=generic", "MARCH=armv8-a+crc"]

        if spec.target.family == "x86_64" or spec.target.family == "x86":
            if spec.target == "x86_64":
                options += ["JULIA_CPU_TARGET=generic"]
            else:
                ...
                if target_str == "zen2":
                    target_str = "znver2"
                options += ["JULIA_CPU_TARGET={0}".format(target_str)]

        if "%intel" in spec:
            options += ["USEICC=1", "USEIFC=1", "USE_INTEL_LIBM=1"]
