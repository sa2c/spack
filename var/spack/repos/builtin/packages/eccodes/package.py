...
class Eccodes(CMakePackage):
    ...
    depends_on("python@2.6:2.999", when="+python", type=("build", "link", "run"))
    ...
    def cmake_args(self):
        var_opt_list = [
            ...
            ("+memfs", "MEMFS"),
            ("+python", "PYTHON2" if self.spec.satisfies("@2.20.0:") else "PYTHON"),
            ("+fortran", "FORTRAN"),
        ]
        ...
        args.append("-DENABLE_JPG=" + ("OFF" if jp2k == "none" else "ON"))
        args.append("-DENABLE_JPG_LIBJASPER=" + ("ON" if jp2k == "jasper" else "OFF"))
        args.append(
            "-DENABLE_JPG_LIBOPENJPEG=" + ("ON" if jp2k == "openjpeg" else "OFF")
        )
