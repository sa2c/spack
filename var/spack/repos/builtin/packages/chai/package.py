...
class Chai(CMakePackage, CudaPackage, ROCmPackage):
    ...
    def cmake_args(self):
        ...
        if "+cuda" in spec:
            options.extend(
                ["-DENABLE_CUDA=ON", "-DCUDA_TOOLKIT_ROOT_DIR=" + spec["cuda"].prefix]
            )
        ...
        if "+raja" in spec:
            options.extend(
                ["-DENABLE_RAJA_PLUGIN=ON", "-DRAJA_DIR=" + spec["raja"].prefix]
            )
        ...
        options.append("-DENABLE_TESTS={0}".format("ON" if "+tests" in spec else "OFF"))
        ...
        options.append(
            "-DENABLE_BENCHMARKS={0}".format("ON" if "+benchmarks" in spec else "OFF")
        )
