...
class Cabana(CMakePackage):
    ...
    def cmake_args(self):
        options = [
            ...
            "-DCabana_ENABLE_Serial=%s" % ("On" if "+serial" in self.spec else "Off"),
            "-DCabana_ENABLE_OpenMP=%s" % ("On" if "+openmp" in self.spec else "Off"),
            "-DCabana_ENABLE_Cuda=%s" % ("On" if "+cuda" in self.spec else "Off"),
            "-DCabana_ENABLE_MPI=%s" % ("On" if "+mpi" in self.spec else "Off"),
