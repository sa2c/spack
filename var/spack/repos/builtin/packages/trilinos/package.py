...
class Trilinos(CMakePackage, CudaPackage):
    ...
    version(
        "13.0.1", commit="4796b92fb0644ba8c531dd9953e7a4878b05c62d"
    ...
    variant("hwloc", default=False, description="Enable hwloc")
    ...
    variant("glm", default=True, description="Compile with GLM")
    ...
    variant("matio", default=True, description="Compile with Matio")
    ...
    variant("netcdf", default=True, description="Compile with netcdf")
    ...
    variant(
        "alloptpkgs", default=False, description="Compile with all optional packages"
    )
    ...
    variant(
        "amesos2basker", default=False, description="Compile with Basker in Amesos2"
    )
    ...
    variant("mesquite", default=False, description="Enable Mesquite")
    ...
    for __compiler in spack.compilers.supported_compilers():
        if __compiler != "clang":
            conflicts(
                "+cuda",
                when="~wrapper %{0}".format(__compiler),
    ...
    def cmake_args(self):
        ...
        def define_trilinos_enable(cmake_var, spec_var=None):
            ...
            return self.define_from_variant("Trilinos_ENABLE_" + cmake_var, spec_var)

        def define_tpl_enable(cmake_var, spec_var=None):
            if spec_var is None:
                spec_var = cmake_var.lower()
            return self.define_from_variant("TPL_ENABLE_" + cmake_var, spec_var)

        def define_prefix_enable(prefix, cmake_var, spec_var=None):
            if spec_var is None:
                spec_var = cmake_var.lower()
            return self.define_from_variant("%s" % prefix, spec_var)
        ...
        options.append(self.define_from_variant("USE_XSDK_DEFAULTS", "xsdkflags"))
        ...
        if "+suite-sparse" in spec:
            options.extend(
                [
                    ...
                    define("UMFPACK_LIBRARY_DIRS", spec["suite-sparse"].prefix.lib),
                    define("UMFPACK_INCLUDE_DIRS", spec["suite-sparse"].prefix.include),
                    define(
                        "UMFPACK_LIBRARY_NAMES",
        ...
        if "+mumps" in spec:
            ...
            options.extend(
                [
                    ...
                    # order is important!
                    define("MUMPS_LIBRARY_NAMES", ["dmumps", "mumps_common", "pord"]),
        ...
        if "+superlu-dist" in spec:
            options.extend(
                [
                    ...
                    define("SuperLUDist_LIBRARY_DIRS", spec["superlu-dist"].prefix.lib),
                    define(
                        "SuperLUDist_INCLUDE_DIRS", spec["superlu-dist"].prefix.include
                    ),
                ]
            )
        ...
        if "+strumpack" in spec:
            options.extend(
                [
                    ...
                    define(
                        "STRUMPACK_LIBRARY_DIRS", spec["strumpack"].libs.directories[0]
                    ),
                    define(
        ...
        options.append(
            define(
                "Kokkos_ARCH_" + Kokkos.spack_micro_arch_map[spec.target.name].upper(),
                True,
            )
        )
        ...
        if "+cuda" in spec:
            ...
            for iArchCC in spec.variants["cuda_arch"].value:
                options.append(
                    define(
                        "Kokkos_ARCH_" + Kokkos.spack_cuda_arch_map[iArchCC].upper(),
                        True,
                    )
                )
            if "+wrapper" in spec:
        ...
        if "+fortran" in spec and (
            ...
            if "+mpi" in spec:
                libgfortran = os.path.dirname(
                    os.popen(
                        "%s --print-file-name libgfortran.a" % spec["mpi"].mpifc
                    ).read()
                )
                options.append(
                    define(
