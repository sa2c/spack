...
class FluxCore(AutotoolsPackage):
    ...
    def setup_run_environment(self, env):
        ...
        env.prepend_path(
            "FLUX_PMI_LIBRARY_PATH", os.path.join(self.prefix.lib.flux, "libpmi.so")
        )
        # Wreck was removed in 0.12
        if self.version < Version("0.12.0"):
            env.prepend_path("FLUX_WREXECD_PATH", self.prefix.libexec.flux.wrexecd)
            env.prepend_path(
    ...
    def flag_handler(self, name, flags):
        if name == "cflags":
            ...
            if self.spec.satisfies("%gcc@10:") and self.spec.satisfies(
                "@0.23.0:0.23.99"
            ):
                if flags is None:
                    flags = []
