...
def test_user_input_combination(config):
    ...
    for target, operating_system in itertools.product(possible_targets, possible_os):
        platform = spack.architecture.platform()
        spec_str = "libelf os={0} target={1}".format(operating_system, target)
        spec = Spec(spec_str)
        spec.concretize()
        assert spec.architecture.os == str(platform.operating_system(operating_system))
...
def test_concretize_target_ranges(root_target_range, dep_target_range, result):
    ...
    spec = Spec(
        "a %%gcc@10 foobar=bar target=%s ^b target=%s"
