...
class PyAzuremlDataprep(Package):
    ...
    depends_on(
        "py-azureml-dataprep-native@30.0.0:30.999",
        when="@2.10.0:",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-dataprep-native@14.2.1:14.999",
        when="@:2.0.2",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-dataprep-rslex@1.9.0:1.9.999",
        when="@2.11.0:",
        type=("build", "run"),
    )
    depends_on(
        "py-azureml-dataprep-rslex@1.8.0:1.8.999", when="@2.10.1", type=("build", "run")
    )
    depends_on("py-cloudpickle@1.1.0:1.999", type=("build", "run"))
    depends_on(
        "py-azure-identity@1.2.0:1.4.999", when="@2.10.0:", type=("build", "run")
    )
