...
class Chombo(MakefilePackage):
    ...
    def edit(self, spec, prefix):
        ...
        defs_file.filter(
            r"^\s*#\s*cppcallsfort\s*=\s*", "cppcallsfort = -DCH_FORT_UNDERSCORE"
        )
        ...
        defs_file.filter(
            r"^\s*#\s*syslibflags\s*=\s*", "syslibflags = %s" % lapack_blas.ld_flags
        )
        ...
        if "+mpi" in spec:
            defs_file.filter(
                r"^\s*#\s*MPICXX\s*=\s*", "MPICXX = %s" % self.spec["mpi"].mpicxx
            )

        # Conditionally determined settings
        defs_file.filter(
            r"^\s*#\s*MPI\s*=\s*", "MPI = %s" % ("TRUE" if "+mpi" in spec else "FALSE")
        )
        defs_file.filter(
            r"^\s*#\s*DIM\s*=\s*", "DIM = %s" % spec.variants["dims"].value
        )
