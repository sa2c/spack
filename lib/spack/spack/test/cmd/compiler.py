...
def no_compilers_yaml(mutable_config):
    ...
    for scope, local_config in mutable_config.scopes.items():
        compilers_yaml = os.path.join(local_config.path, scope, "compilers.yaml")
...
def test_compiler_add(
    ...
    assert any(
        c.version == spack.version.Version(mock_compiler_version) for c in new_compiler
    )
...
def test_compiler_find_mixed_suffixes(no_compilers_yaml, working_env, clangdir):
    ...
    clang = next(
        c["compiler"] for c in config if c["compiler"]["spec"] == "clang@11.0.0"
    )
