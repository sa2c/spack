...
""".format(
    ...
    with spack.config.override(test_scope):
        ...
        assert any(path_includes_fake_prefix(cmd) for cmd in individual_modifications)
...
def test_stack_view_activate_from_default(
    tmpdir, mock_fetch, mock_packages, mock_archive, install_mockery, env_deactivate
):
    filename = str(tmpdir.join("spack.yaml"))
    viewdir = str(tmpdir.join("view"))
    with open(filename, "w") as f:
