...
class Spec(object):
    ...
    def _new_concretize(self, tests=False):
        ...
        if not result.satisfiable:
            result.print_cores()
            raise spack.error.UnsatisfiableSpecError(self, "unknown", "Unsatisfiable!")
    ...
    def splice(self, other, transitive):
        ...
        if transitive:
            ...
            other_nodes = dict(
                (s.name, s.copy(deps=False)) for s in other.traverse(root=True)
            )
        else:
...
def save_dependency_spec_yamls(root_spec_as_yaml, output_directory, dependencies=None):
