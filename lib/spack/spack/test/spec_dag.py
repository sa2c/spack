...
def test_installed_deps():
    ...
    c = mock_repo.add_package(
        "c", [d, e], [build_only, default], conditions=c_conditions
    )
...
class TestSpecDag(object):
    ...
    def test_conflicting_spec_constraints(self):
        ...
        mpileaks._dependencies["callpath"].spec._dependencies["mpich"].spec = Spec(
            "mpich@2.0"
        )
    ...
    def test_unsatisfiable_architecture(self, set_dependency):
        ...
        spec = Spec(
            "mpileaks ^mpich platform=test target=fe ^callpath"
    ...
    def test_equal(self):
        ...
        dag = Spec.from_literal(
            {"mpileaks": {"callpath": {"libdwarf": {"libelf": None}}}}
        )
    ...
    def test_hash_bits(self):
        ...
        for test_hash in tests:
            ...
            expected = "".join(
                [format(b32[c], "#07b").replace("0b", "") for c in test_hash]
            )
    ...
    def check_diamond_deptypes(self, spec):
        ...
        assert spec["dt-diamond"]._dependencies["dt-diamond-right"].deptypes == (
            "build",
            "link",
        )

        assert spec["dt-diamond-left"]._dependencies["dt-diamond-bottom"].deptypes == (
            "build",
        )

        assert spec["dt-diamond-right"]._dependencies["dt-diamond-bottom"].deptypes == (
            "build",
            "link",
            "run",
        )
