...
def config_directory(tmpdir_factory):
    ...
    os_modules_yaml = config_path.join(
        "spack", "defaults", "%s" % platform.system().lower(), "modules.yaml"
    )
    ...
    os_modules_yaml.copy(
        tmpdir.join("site/%s" % platform.system().lower(), "modules.yaml")
    )
...
@pytest.fixture(scope="function")
def default_config(
    tmpdir_factory, config_directory, monkeypatch, install_mockery_mutable_config
):
...
def test_push_and_fetch_keys(mock_gnupghome):
    ...
    with spack.util.gpg.gnupghome_override(gpg_dir1):
        spack.util.gpg.create(
            name="test-key", email="fake@test.key", expires="0", comment=None
        )
