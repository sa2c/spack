...
class PyTorch(PythonPackage, CudaPackage):
    ...
    depends_on(
        "python@2.7:2.8,3.5:", when="@1.4.0:1.4.999", type=("build", "link", "run")
    )
    depends_on(
        "python@2.7:2.8,3.5:3.7.999", when="@:1.3.999", type=("build", "link", "run")
    )
    ...
    depends_on(
        "py-pybind11@2.6.2", when="@1.8.0:1.9.999", type=("build", "link", "run")
    )
    depends_on(
        "py-pybind11@2.3.0", when="@1.1.0:1.7.999", type=("build", "link", "run")
    )
    depends_on(
        "py-pybind11@2.2.4", when="@1.0.0:1.0.999", type=("build", "link", "run")
    )
    depends_on(
        "py-pybind11@2.2.2", when="@0.4.0:0.4.999", type=("build", "link", "run")
    )
    depends_on(
        "py-dataclasses", when="@1.7: ^python@3.6.0:3.6.999", type=("build", "run")
    )
