...
def setup_parser(subparser):
    ...
    getbuildcachename = subparsers.add_parser(
        "get-buildcache-name", help=get_buildcache_name.__doc__
    )
    ...
    saveyaml = subparsers.add_parser("save-yaml", help=save_spec_yamls.__doc__)
...
def match_downloaded_specs(
    pkgs, allow_multiple_matches=False, force=False, other_arch=False
):
    """Returns a list of specs matching the not necessarily
       concretized specs given from cli
    ...
    for match in matches:
        ...
        if match.external or match.virtual:
            ...
            if not add_spec:
                ...
                tty.debug("skipping uninstalled matching spec %s" % match.format())
            else:
            ...
            for d, node in match.traverse(
                order="post", depth=True, deptype=("link", "run")
            ):
                ...
                if node.external or node.virtual:
                    tty.debug(
                        "skipping external or virtual dependency %s" % node.format()
                    )
                elif lookup is None:
                    tty.debug("skipping uninstalled depenendency %s" % node.format())
                else:
...
def createtarball(args):
    ...
    if args.directory:
        ...
        if scheme != "<missing>":
            raise ValueError('"--directory" expected a local path; got a URL, instead')
        ...
        if result.name == "<unnamed>":
            raise ValueError(
                'no configured mirror named "{name}"'.format(name=output_location)
            )
        ...
        scheme = url_util.parse(output_location, scheme="<missing>").scheme
        if scheme == "<missing>":
            raise ValueError('"{url}" is not a valid URL'.format(url=output_location))
...
def installtarball(args):
    ...
    if not args.specs:
        tty.die(
            "build cache file installation requires"
...
def install_tarball(spec, args):
    ...
    if s.concrete and package.installed and not args.force:
        ...
        if tarball:
            ...
            bindist.extract_tarball(
                spec, tarball, args.allow_root, args.unsigned, args.force
            )
...
def download_buildcache_files(
    concrete_spec, local_dest, require_cdashid, mirror_url=None
):
...
def get_concrete_spec(args):
    ...
    if not spec_str and not spec_yaml_path:
        tty.msg(
            "Must provide either spec string or path to " + "yaml to concretize spec"
        )
        sys.exit(1)
...
def save_spec_yamls(args):
    ...
    save_dependency_spec_yamls(root_spec_as_yaml, args.yaml_dir, args.specs.split())
