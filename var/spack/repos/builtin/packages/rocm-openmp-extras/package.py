...
class RocmOpenmpExtras(Package):
    ...
    for ver in ["3.9.0", "3.10.0", "4.0.0", "4.1.0", "4.2.0"]:
        ...
        depends_on(
            "llvm-amdgpu@{0} ~rocm-device-libs ~openmp".format(ver), when="@" + ver
        )
    ...
    def install(self, spec, prefix):
        ...
        if not (os.path.islink((os.path.join(bin_dir, "flang1")))):
            os.symlink(
                os.path.join(omp_bin_dir, "flang1"), os.path.join(bin_dir, "flang1")
            )
        if not (os.path.islink((os.path.join(bin_dir, "flang2")))):
            os.symlink(
                os.path.join(omp_bin_dir, "flang2"), os.path.join(bin_dir, "flang2")
            )
        if not (os.path.islink((os.path.join(lib_dir, "libdevice")))):
            os.symlink(
                os.path.join(omp_lib_dir, "libdevice"),
