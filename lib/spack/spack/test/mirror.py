...
def test_all_mirror(
    mock_git_repository, mock_svn_repository, mock_hg_repository, mock_archive
):
...
def test_mirror_with_url_patches(mock_packages, config, monkeypatch):
    ...
    with Stage("spack-mirror-test") as stage:
        ...
        monkeypatch.setattr(
            spack.fetch_strategy.URLFetchStrategy, "fetch", successful_fetch
        )
        monkeypatch.setattr(
            spack.fetch_strategy.URLFetchStrategy, "expand", successful_expand
        )
...
def test_mirror_cache_symlinks(tmpdir):
    ...
    assert os.path.normpath(link_target) == os.path.join(
        cache.root, reference.storage_path
    )
