...
class Umpire(CMakePackage, CudaPackage, ROCmPackage):
    ...
    for sm_ in CudaPackage.cuda_arch_values:
        depends_on("camp cuda_arch={0}".format(sm_), when="cuda_arch={0}".format(sm_))
    ...
    def cmake_args(self):
        ...
        options.append("-DENABLE_C={0}".format("On" if "+c" in spec else "Off"))

        options.append(
            "-DENABLE_FORTRAN={0}".format("On" if "+fortran" in spec else "Off")
        )

        options.append("-DENABLE_NUMA={0}".format("On" if "+numa" in spec else "Off"))

        options.append(
            "-DENABLE_OPENMP={0}".format("On" if "+openmp" in spec else "Off")
        )

        options.append(
            "-DBUILD_SHARED_LIBS={0}".format("On" if "+shared" in spec else "Off")
        )

        options.append(
            "-DENABLE_BENCHMARKS={0}".format(
                "On" if "tests=benchmarks" in spec else "Off"
            )
        )

        options.append(
            "-DENABLE_EXAMPLES={0}".format("On" if "+examples" in spec else "Off")
        )

        options.append(
            "-DENABLE_TESTS={0}".format("Off" if "tests=none" in spec else "On")
        )
