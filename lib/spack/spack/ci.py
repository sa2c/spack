...
def generate_gitlab_ci_yaml(
        ...
        for phase in phases:
            ...
            with spack.concretize.disable_compiler_existence_check():
                staged_phases[phase_name] = stage_spec_jobs(
                    env.spec_lists[phase_name], check_index_only=check_index_only
                )
    finally:
    ...
    if job_id > 0:
        ...
        if rebuild_index_enabled:
            ...
            if artifacts_root:
                final_job["variables"] = {"SPACK_CONCRETE_ENV_DIR": concrete_env_dir}
...
def push_mirror_contents(env, spec, yaml_path, mirror_url, sign_binaries):
    try:
        ...
        tty.debug(
            "Creating buildcache ({0})".format("unsigned" if unsigned else "signed")
        )
