...
class Ceed(BundlePackage, CudaPackage, ROCmPackage):
    ...
    for target in ROCmPackage.amdgpu_targets:
        depends_on(
            "petsc@3.15.0:3.15.99+rocm amdgpu_target={0}".format(target),
            when="@4.0.0:4.99.99+petsc+rocm amdgpu_target={0}".format(target),
        )
    depends_on("petsc@3.15.0:3.15.99~hdf5~superlu-dist", when="@4.0.0+petsc+quickbuild")
    depends_on(
        "petsc@3.15.0:3.15.99+mpi+double~int64", when="@4.0.0:4.99.99+petsc~mfem"
    )
    # ceed-3.0
    depends_on("petsc+cuda", when="@3.0.0+petsc+cuda")
    # For a +quickbuild we disable hdf5, and superlu-dist in PETSc.
    depends_on("petsc@3.13.0:3.13.99~hdf5~superlu-dist", when="@3.0.0+petsc+quickbuild")
    ...
    # and +mumps:
    depends_on(
        "petsc@3.13.0:3.13.99+mpi+hypre+suite-sparse+mumps+double~int64",
