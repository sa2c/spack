...
def test_fetch(
    type_of_test, secure, mock_git_repository, config, mutable_mock_repo, git_version
):
    """Tries to:
...
def test_get_full_repo(
    get_full_repo, git_version, mock_git_repository, config, mutable_mock_repo
):
    ...
    with pkg.stage:
        with spack.config.override("config:verify_ssl", secure):
            pkg.do_stage()
            with working_dir(pkg.stage.source_path):
                branches = mock_git_repository.git_exe(
                    "branch", "-a", output=str
                ).splitlines()
...
def test_gitsubmodule(submodules, mock_git_repository, config, mutable_mock_repo):
    """
...
def test_gitsubmodules_delete(mock_git_repository, config, mutable_mock_repo):
    ...
    pkg.do_stage()
    with working_dir(pkg.stage.source_path):
        file_path = os.path.join(pkg.stage.source_path, "third_party/submodule0")
        assert not os.path.isdir(file_path)
        file_path = os.path.join(pkg.stage.source_path, "third_party/submodule1")
        assert not os.path.isdir(file_path)
