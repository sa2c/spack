    ...
    for spec, mod in modules:
        if not mod:
            module_output_for_spec = (
                "## blacklisted or missing from upstream: {0}".format(spec.format())
            )
        else:
...
def find(module_type, specs, args):
    ...
    if args.recurse_dependencies:
        dependency_specs_to_retrieve = list(
            single_spec.traverse(
                root=False, order="post", cover="nodes", deptype=("link", "run")
            )
        )
    else:
...
def modules_cmd(parser, args, module_type, callbacks=callbacks):
    ...
    try:
        ...
        tty.die(
            "In this context exactly **one** match is needed: please specify your constraints better."
        )  # NOQA: ignore=E501
