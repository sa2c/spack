...
class TestTcl(object):
    ...
    def test_alter_environment(self, modulefile_content, module_configuration):
        ...
        assert (
            len([x for x in content if x.startswith("prepend-path CMAKE_PREFIX_PATH")])
            == 0
        )
        assert len([x for x in content if 'setenv FOO "foo"' in x]) == 1
        assert (
            len([x for x in content if 'setenv OMPI_MCA_mpi_leave_pinned "1"' in x])
            == 1
        )
        assert (
            len([x for x in content if 'setenv OMPI_MCA_MPI_LEAVE_PINNED "1"' in x])
    ...
    def test_config_backwards_compat(self, mutable_config):
        settings = {"enable": ["tcl"], "tcl": {"all": {"conflict": ["{name}"]}}}
