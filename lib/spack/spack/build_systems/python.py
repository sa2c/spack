...
class PythonPackage(PackageBase):
    ...
    * install_data
    ...
    def install_args(self, spec, prefix):
        ...
        if (
            "py-setuptools" == spec.name
            or "py-setuptools" in spec._dependencies  # this is setuptools, or
            and "build"  # it's an immediate dep
            in spec._dependencies["py-setuptools"].deptypes
        ):
        ...
        pure_site_packages_dir, plat_site_packages_dir, inc_dir = (
            python("-c", commands, output=str, error=str).strip().split("\n")
        )
    ...
    def remove_files_from_view(self, view, merge_map):
        ...
        bin_dir = self.spec.prefix.bin
        global_view = self.extendee_spec.prefix == view.get_projection_for_spec(
            self.spec
        )
        for src, dst in merge_map.items():
