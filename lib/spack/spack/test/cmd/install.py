...
def test_extra_files_are_archived(
    ...
    config_log = os.path.join(
        archive_dir, mock_archive.expanded_archive_basedir, "config.log"
    )
...
def test_cdash_report_concretization_error(
    ...
    with capfd.disabled():
        with tmpdir.as_cwd():
            ...
            expected_messages = ("Conflicts in concretized spec", "does not satisfy")
...
def test_cdash_buildstamp_param(tmpdir, mock_fetch, install_mockery, capfd):
    ...
    with capfd.disabled():
        with tmpdir.as_cwd():
            ...
            buildstamp = time.strftime(
                buildstamp_format, time.localtime(int(time.time()))
            )
            install(
                "--log-file=cdash_reports",
                "--log-format=cdash",
...
@pytest.mark.disable_clean_stage_check
def test_cdash_install_from_spec_yaml(
...
def test_cdash_configure_warning(tmpdir, mock_fetch, install_mockery, capfd):
    ...
    with capfd.disabled():
        with tmpdir.as_cwd():
            ...
            install(
                "--log-file=cdash_reports", "--log-format=cdash", "configure-warning"
            )
...
def test_compiler_bootstrap_from_binary_mirror(
    ...
    # raise an error
    install(
        "--no-check-signature", "--cache-only", "--only", "dependencies", "b%gcc@10.2.0"
    )
...
def test_cache_install_full_hash_match(
    ...
    expect_msg = "No binary for {0} found: installing from source".format(package_id)
