...
class BinaryCacheIndex(object):
    ...
    def _fetch_and_cache_index(self, mirror_url, expect_hash=None):
        ...
        try:
            ...
            tty.debug(
                "Unable to read index hash {0}".format(hash_fetch_url), url_err, 1
            )
    ...
    for root, dirs, files in os.walk(spec.prefix, topdown=True):
        ...
        for filename in files:
            ...
            if relocate.needs_binary_relocation(m_type, m_subtype):
                if (
                    (
                        m_subtype in ("x-executable", "x-sharedlib")
...
def generate_package_index(cache_prefix):
    ...
    tty.debug("Retrieving spec.yaml files from {0} to build index".format(cache_prefix))
...
def try_direct_fetch(spec, full_hash_match=False, mirrors=None):
    ...
    for mirror in spack.mirror.MirrorCollection(mirrors=mirrors).values():
        ...
        try:
            ...
            tty.debug(
                "Did not find {0} on {1}".format(specfile_name, buildcache_fetch_url),
                url_err,
            )
            continue
