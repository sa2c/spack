    ...
    def test(self):
        ...
        with open(cmake_file, "w") as cmkf:
            ...
            if "+fftw" in self.spec:
                src_file = "{0}/heffte_example_fftw.cpp".format(self.examples_src_dir)
                cmkf.write("add_executable(hf_fftw {0})\n".format(src_file))
                cmkf.write("target_link_libraries(hf_fftw Heffte::Heffte)\n")
                exe_files.append("./hf_fftw")
                mpi_procs.append(4)
            if "+cuda" in self.spec:
                src_file = "{0}/heffte_example_cuda.cpp".format(self.examples_src_dir)
                cmkf.write("add_executable(hf_cuda {0})\n".format(src_file))
                cmkf.write("target_link_libraries(hf_cuda Heffte::Heffte)\n")
                exe_files.append("./hf_cuda")
                mpi_procs.append(4)
            if "+fortran" in self.spec:
                src_file = "{0}/heffte_example_fftw.f90".format(self.examples_src_dir)
                cmkf.write("add_executable(hf_fort {0})\n".format(src_file))
                cmkf.write("target_link_libraries(hf_fort Heffte::Fortran)\n")
        ...
        with working_dir(test_bld_dir, create=True):
            ...
            if mpiexe_f:
                for exf, procs in zip(exe_files, mpi_procs):
                    ...
                    self.run_test(mpiexe_f.command, test_args, purpose=reason_str)
