...
class Scr(CMakePackage):
    ...
    def cmake_args(self):
        ...
        if spec.satisfies("@:2.999"):
            args.append(
                "-DSCR_ASYNC_API={0}".format(spec.variants["async_api"].value.upper())
            )

        args.append(
            "-DSCR_FILE_LOCK={0}".format(spec.variants["file_lock"].value.upper())
        )

        args.append("-DSCR_CACHE_BASE={0}".format(spec.variants["cache_base"].value))

        args.append("-DSCR_CNTL_BASE={0}".format(spec.variants["cntl_base"].value))
