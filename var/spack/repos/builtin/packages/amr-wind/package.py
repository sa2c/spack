...
class AmrWind(CMakePackage, CudaPackage):
    ...
    variant("unit", default=True, description="Build unit tests")
    ...
    variant("internal-amrex", default=True, description="Use AMRex submodule to build")
    variant("fortran", default=False, description="Build fortran interfaces")
    ...
    for arch in CudaPackage.cuda_arch_values:
        depends_on(
            "hypre+mpi~int64+cuda cuda_arch=%s @2.20.0:" % arch,
