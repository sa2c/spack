...
class CbtfArgonavis(CMakePackage):
    ...
    depends_on(
        "mrnet@5.0.1-3:+lwthreads", when="@develop~cti", type=("build", "link", "run")
    )
    depends_on(
        "mrnet@5.0.1-3+cti", when="@1.9.3:9999+cti", type=("build", "link", "run")
    )
    depends_on(
        "mrnet@5.0.1-3+lwthreads", when="@1.9.3:9999~cti", type=("build", "link", "run")
    )
    ...
    depends_on(
        "cbtf@1.9.3:9999+cti", when="@1.9.3:9999+cti", type=("build", "link", "run")
    )
