...
def test_dev_build_before_until(tmpdir, mock_packages, install_mockery, capsys):
    ...
    with tmpdir.as_cwd():
        ...
        out = dev_build(
            "-u", bad_phase, "dev-build-test-install@0.0.0", fail_on_error=False
        )
        ...
        out = dev_build(
            "-b", bad_phase, "dev-build-test-install@0.0.0", fail_on_error=False
        )
        assert bad_phase in out
        assert not_allowed in out
        assert not_installed in out
...
def test_dev_build_drop_in(tmpdir, mock_packages, monkeypatch, install_mockery):
    ...
    with tmpdir.as_cwd():
        output = dev_build(
            "-b", "edit", "--drop-in", "sh", "dev-build-test-install@0.0.0"
        )
...
@pytest.mark.parametrize(
    "test_spec", ["dev-build-test-install", "dependent-of-dev-build"]
)
