...
def get_monitor_group(subparser):
    ...
    monitor_group.add_argument(
        "--monitor-save-local",
        action="store_true",
        dest="monitor_save_local",
        default=False,
        help="save monitor results to .spack instead of server.",
    )
    monitor_group.add_argument(
        "--monitor-no-auth",
        action="store_true",
        dest="monitor_disable_auth",
        default=False,
        help="the monitoring server does not require auth.",
    )
    monitor_group.add_argument(
        "--monitor-tags",
        dest="monitor_tags",
    ...
    monitor_group.add_argument(
        "--monitor-keep-going",
        action="store_true",
        dest="monitor_keep_going",
        default=False,
        help="continue the build if a request to monitor fails.",
    )
    monitor_group.add_argument(
        "--monitor-host",
        dest="monitor_host",
...
class SpackMonitorClient:
    ...
    def __init__(
        self, host=None, prefix="ms1", allow_fail=False, tags=None, save_local=False
    ):
            ...
            if hasattr(e, "code") and e.code == 401 and retry:
                if self.authenticate_request(e):
                    request = self.prepare_request(
                        e.url, sjson.load(request.data.decode("utf-8")), self.headers
                    )
    ...
    def new_configuration(self, specs):
        ...
        for spec in specs:
            ...
            if spec.name in spack.repo.path or spec.virtual:
                spec.concretize()
            as_dict = {
                "spec": spec.to_dict(hash=ht.full_hash),
    ...
    def send_analyze_metadata(self, pkg, metadata):
        ...
        data = {
            "build_id": self.get_build_id(pkg.spec, spec_exists=False),
            "metadata": metadata,
        }
