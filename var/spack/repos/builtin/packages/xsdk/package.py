...
class Xsdk(BundlePackage):
    ...
    depends_on(
        "mfem@4.2.0+mpi+superlu-dist+petsc+sundials+examples+miniapps", when="@0.6.0"
    )
    depends_on(
    ...
    depends_on(
        "petsc@xsdk-0.2.0+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64",
        when="@xsdk-0.2.0",
    )
    ...
    depends_on(
        "tasmanian@develop+xsdkflags+blas+cuda+magma" + tasmanian_openmp,
        when="@develop +cuda",
    )
    depends_on("tasmanian@7.3+xsdkflags+mpi+blas" + tasmanian_openmp, when="@0.6.0")
    depends_on(
        "tasmanian@7.3+xsdkflags+mpi+blas+cuda+magma" + tasmanian_openmp,
        when="@0.6.0 +cuda",
    )
    depends_on("tasmanian@7.0+xsdkflags+mpi+blas" + tasmanian_openmp, when="@0.5.0")
    depends_on(
        "tasmanian@7.0+xsdkflags+mpi+blas+cuda+magma" + tasmanian_openmp,
        when="@0.5.0 +cuda",
    )
    ...
    depends_on(
        "py-libensemble@develop+petsc4py", type="run", when="@develop +libensemble"
    )
