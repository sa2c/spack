...
def mock_pkg_install(monkeypatch):
    monkeypatch.setattr(
        spack.package.PackageBase, "install", _pkg_install_fn, raising=False
    )
...
@pytest.fixture(scope="session")
def mock_store(
    tmpdir_factory, mock_repo_path, mock_configuration_scopes, _store_dir_and_cache
):
...
def mock_fetch(mock_archive, monkeypatch):
    ...
    monkeypatch.setattr(spack.package.PackageBase, "fetcher", mock_fetcher)
...
class ConfigUpdate(object):
    ...
    def __call__(self, filename):
        ...
        self.monkeypatch.setattr(
            spack.modules.common, "configuration", mock_config.configuration
        )
        self.monkeypatch.setattr(
...
def mock_archive(request, tmpdir_factory):
    ...
    with tmpdir.as_cwd():
        archive_name = "{0}{1}".format(
            spack.stage._source_path_subdir, request.param[0]
        )
        tar(
            "-c{0}f".format(request.param[1]),
            archive_name,
...
def mock_git_repository(tmpdir_factory):
    ...
    checks = {
        "master": Bunch(revision="master", file=r0_file, args={"git": url}),
        "branch": Bunch(
            revision=branch, file=branch_file, args={"git": url, "branch": branch}
        ),
        "tag-branch": Bunch(
    ...
    t = Bunch(checks=checks, url=url, hash=rev_hash, path=str(repodir), git_exe=git)
    yield t
...
def mock_hg_repository(tmpdir_factory):
    ...
    checks = {
        ...
        "rev0": Bunch(
            revision=r0, file=r0_file, args={"hg": str(repodir), "revision": r0}
        ),
    }
