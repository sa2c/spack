...
def activate(env, use_env_repo=False, add_view=True, shell="sh", prompt=None):
...
class Environment(object):
    ...
    def _spec_needs_overwrite(self, spec):
        ...
        if any(
            self._spec_needs_overwrite(dep)
            or ((not dep.package.installed) and dep.satisfies("dev_path=*"))
    ...
    def _get_overwrite_specs(self):
        ...
        for dag_hash in self.concretized_order:
            ...
            ret.extend(
                [
                    d.dag_hash()
                    for d in spec.traverse(root=True)
    ...
    def install_specs(self, specs=None, args=None, **install_args):
        ...
        install_args["overwrite"] = (
            install_args.get("overwrite", []) + self._get_overwrite_specs()
        )
    ...
    def _read_lockfile_dict(self, d):
        ...
        for dag_hash, node_dict in json_specs_by_hash.items():
            for dep_name, dep_hash, deptypes in Spec.dependencies_from_node_dict(
                node_dict
            ):
